...
export class LabHomeComponent implements OnInit {
  ...
  constructor(private LabHomeService: LabHomeService,private http:HttpClient) {}
  ...
  upload(id: string){
    ...
    this.http.post<any>('http://localhost:8000/api/lab/upload/'+id + "/"+sessionStorage.getItem('uid'), formData).subscribe(
      (res) => console.log(res),
      (err) => console.log(err)
    );
  }
