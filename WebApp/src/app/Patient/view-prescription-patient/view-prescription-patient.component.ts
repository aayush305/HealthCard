...
export class ViewPrescriptionPatientComponent implements OnInit {
  ...
  getPrescriptionList()
  {
    this.http.get(this.registerService.UserUrl+"/getDiagnosisList/"+this.userId)
    .subscribe((res:any)=>{
  ...
  getDetails(pId,reportIds){
    this.http.get(this.registerService.UserUrl+"/getDetails/"+pId+"/"+reportIds).subscribe((res:any)=>{
